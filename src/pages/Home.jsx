...
const Home = () => {
  ...
  const { data: dataPlaces, isLoading: isLoadingPlaces, isError: isErrorPlaces, error: errorPlaces } = useQuery({
    queryKey: ['places'],
    queryFn: async () => {
  ...
  const { data: dataServices, isLoading: isLoadingServices, isError: isErrorServices, error: errorServices } = useQuery({
    queryKey: ['services'],
    queryFn: async () => {
  ...
  if (isErrorPlaces || isErrorServices) {
    return (
      <div style={{ padding: '20px', textAlign: 'center' }}>
        ...
        <button onClick={() => window.location.reload()} style={{ padding: '10px 20px', backgroundColor: 'blue', color: 'white' }}>
          Tentar Novamente
        </button>
      </div>
    );
